...
object UserManager : IUserManager {
     ...
     * @param[user] 用户数据
     */
    private fun defaultLogin(user: UserData) =
            // 关注消息推送中用户订阅的频道
...
//        }
     ...
     * @param[user] 用户数据
     */
    private fun defaultLogout(user: UserData) =
...
//        }
     ...
     */
    private fun makeAVPassword(username: String, seed: String): String = getSHA("$username$seed")
     ...
     * @return 设备存储的用户名
     */
    private fun getUsername(): String = mSP?.getString("username", "") ?: ""

    /**
     * 判断是否登录成功过
     * @return 是否登录成功过
     */
    private fun checkLoggedIn(): Boolean = mSP?.getBoolean("loggedin", false) ?: false

    /**
     * 判断是否已登录
     * @return 是否已登录
     */
    override fun isLoggedIn(): Boolean = loginFlag
    ...
    // 注销
    override fun logout(process: ((UserData) -> Unit)?): Boolean {
        // 若为登录状态则执行注销
        if (isLoggedIn()) {
            // 根据用户名查找当前登录的用户数据
    ...
    override fun register(username: String, password: String, nickname: String,
        ...
        query.countInBackground(object : CountCallback() {
            override fun done(count: Int, e: AVException?) = if (e == null) {
                if (count > 0) {
                    // 该用户名已存在
                    ...
                    query2.countInBackground(object : CountCallback() {
                        override fun done(i: Int, e: AVException?) = if (e == null) {
                            if (i > 0) {
                                // 昵称已存在
                                ...
                                // 保存数据
                                post.saveInBackground(object : SaveCallback() {
                                    override fun done(e: AVException?) = if (e == null) {
                                        // 保存成功
                                        // 注册AVUser
    ...
    override fun autoLogin(success: (UserData) -> Unit,
        ...
        query.whereEqualTo("username", username)
        query.findInBackground(object : FindCallback<AVObject>() {
            override fun done(list: List<AVObject>?, e: AVException?) = if (e == null) {
                if (list!!.isNotEmpty()) {
                    // 获取该用户名的用户数据
                    val user = list[0]
                    val data = UserData.convert(user)
